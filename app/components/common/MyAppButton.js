...
function MyAppButton({
    ...
    return (
        <TouchableOpacity activeOpacity={0.9} onPress={onPress} style={{ width: width, backgroundColor: backgroundColor }} >
            <LinearGradient colors={gradient ? ['#2B217F', '#2B217F'] : ["#fff", '#fff']} start={[1, 1]} end={[0.2, 0.3]} style={{ opacity: opacity, borderRadius: borderRadius, alignSelf: 'center', alignItems: "center", flexDirection: 'row', justifyContent: 'center', width: width, height: RFPercentage(7), borderColor: gradient ? null : borderColor, borderWidth: gradient ? null : borderWidth }} >
